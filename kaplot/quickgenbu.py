...
def indexedimage(data2d, matrix=None, mask2d=None, colormap='rainbow', datamin=None, datamax=None, function='linear', lock=True, context=None, container=None, **kwargs):
	...
	if container is None:
		_checkContainer()
		container = current.container
	current.object = kaplot.objects.IndexedImage(container, data2d, matrix=matrix, mask2d=mask2d, colormap=colormap, datamin=datamin, datamax=datamax, function=function, lock=lock, context=context, **kwargs)
	return current.object
...
def symbols(x, y, symbolName='dot', xscales=None, yscales=None, angles=None, colors=None, colormap='rainbow', container=None, **kwargs):
	...
	if container is None:
		_checkContainer()
		container = current.container
	current.object = kaplot.objects.Symbols(container, x, y, symbolName=symbolName, xscales=xscales, yscales=yscales, angles=angles, colors=colors, colormap=colormap, **kwargs)
	return current.object
