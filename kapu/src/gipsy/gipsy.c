...
PyObject* PyKaplot_Gipsy_Sky(PyObject* self, PyObject* args)
{
	...
	{
		/*if(PyNumber_Check(x_array) && PyNumber_Check(y_array)) {
			//printf("is float\n");
			is_scalar = TRUE;
			x_scalar[0] = PyFloat_AsDouble(x_array);
			y_scalar[0] = PyFloat_AsDouble(x_array);
			x = x_scalar;
			y = y_scalar;
		} else {*/
			...
			if(y_array_double->nd != 0) {
				...
				x_array_double_out = (PyArrayObject*)PyArray_FromDims(0, dims, tFloat64);
				y_array_double_out = (PyArrayObject*)PyArray_FromDims(0, dims, tFloat64);
			}
...
init_gipsy(void)
{
	...
	import_array();
	//import_libnumeric();
	mod = Py_InitModule("kapu._gipsy", kaplot_gipsy_functions);
