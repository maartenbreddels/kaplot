...
"""
#from kaplot.cext._wcslib import *
from numpy import *
...
def fromDict(header, fixcd=False):
	...
	if fixcd:
		kaplot.debug("WARNING: had to fix the cd matrix")
		cdmatrix = [cdmatrix[0] * cdelt[0], cdmatrix[1] * cdelt[1], cdmatrix[2] * cdelt[0], cdmatrix[3] * cdelt[1]]

	return Projection(crval, ctype, pcmatrix, cdelt, crpix, crota, cdmatrix, altlin)


class Projection:
	def __init__(self, crval, ctype, pcmatrix, cdelt, crpix, crota, cdmatrix, altlin):
		"""
		All arguments correspond to the FITS keywords

		Arguments:
		cvral -- array of [longref, latref]
		ctype -- array of string types example: ["RA---TAN", "DEC--TAN"]
			projection type must be the same as projectiontype paramater
		pcmatrix -- pc matrix
		cdelt -- array of deltas
		crpix -- array


		"""
		if "COE" in ctype[1]:
			self.wcs = Wcs(crval, ctype, pcmatrix, cdelt, crpix, crota, cdmatrix, altlin, [(1,0,1), (2,1,1)])
		else:
			self.wcs = Wcs(crval, ctype, pcmatrix, cdelt, crpix, crota, cdmatrix, altlin)

	def forwardarray(self, longitudes, latitudes):
		x = []
		y = []
		for longitude, latitude in zip(longitudes, latitudes):
			p = self.forward(longitude, latitude)
			if p != None:
				x_, y_ = p
				x.append(x_)
				y.append(y_)
	...
	def forward(self, longitude, latitude):
		return self.wcs.forward(longitude, latitude);

	def reversearray(self, longitudes, latitudes):
		x = []
		y = []
		for longitude, latitude in zip(longitudes, latitudes):
			p = self.reverse(longitude, latitude)
			if p != None:
				x_, y_ = p
				x.append(x_)
				y.append(y_)
